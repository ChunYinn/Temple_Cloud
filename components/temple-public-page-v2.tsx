...
const InstagramIcon = (props: React.SVGProps<SVGSVGElement>) => {
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" aria-hidden="true" {...props}>
      ...
      <rect x="6" y="6" width="36" height="36" rx="10" fill={`url(#${gradientId})`} />
...
const SocialMediaLinks = ({
  ...
  variant?: 'icons' | 'buttons' | 'list';
}) => {
  const hasSocialMedia = facebook || line || instagram;
  if (!hasSocialMedia) return null;

  const socialLinks = [
    {
      url: processSocialMediaUrl(facebook, 'facebook'),
      icon: FacebookIcon,
      label: 'Facebook 粉絲專頁',
    ...
    {
      url: processSocialMediaUrl(line, 'line'),
      icon: LineIcon,
      label: 'LINE 官方帳號',
    ...
    {
      url: processSocialMediaUrl(instagram, 'instagram'),
      icon: InstagramIcon,
      label: 'Instagram',
      colorClass: 'bg-gradient-to-br from-purple-600 via-pink-600 to-orange-600 hover:opacity-90',
      ...
      <div className="flex justify-center gap-4">
        {socialLinks.map((link, idx) => (
          <a
            ...
            className={`w-12 h-12 ${link.colorClass} text-white rounded-full flex items-center justify-center shadow-lg transition-all hover:scale-110`}
    ...
    <div className="space-y-2">
      {socialLinks.map((link, idx) => (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className={`w-full py-3 px-4 rounded-xl font-medium flex items-center gap-3 border border-stone-200 text-stone-700 transition-colors ${link.hoverClass}`}
        >
...
const ContactInfo = ({
  ...
  const infoItems = [
    { icon: '📍', label: '地址', value: temple.address || "台灣", canCopy: true, color: 'red' },
    { icon: '⏰', label: '開放時間', value: temple.hours || TEMPLE_DEFAULTS.hours, color: 'amber' },
    { icon: '📞', label: '聯絡電話', value: temple.phone, color: 'emerald' },
    { icon: '📧', label: '電子信箱', value: temple.email, color: 'blue' }
          ...
          <div key={idx} className="p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-start gap-2">
                <p className="text-stone-700 flex-1">{item.value}</p>
                {showCopy && item.canCopy && item.value !== "台灣" && (
                  <button
                    onClick={() => handleCopyAddress(item.value!)}
    ...
    <div className="space-y-5">
      {infoItems.map((item, idx) => (
        <div key={idx} className="flex gap-4">
          <div className={`flex-shrink-0 w-10 h-10 rounded-full bg-${item.color}-50 flex items-center justify-center text-lg`}>
            {item.icon}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-stone-600 mb-1">{item.label}</p>
            <div className="flex items-start gap-2">
              <p className="text-base text-stone-900 break-words flex-1">{item.value}</p>
              {showCopy && item.canCopy && item.value !== "台灣" && (
                <button
                  onClick={() => handleCopyAddress(item.value!)}
...
const EventCard = ({ event, variant = 'default' }: { event: any; variant?: 'default' | 'compact' | 'detailed' }) => {
  const dateMonth = event.date.split("/")[1];
  const dateDay = event.date.split("/")[2];
    ...
    return (
      ...
      >
        <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-red-500 to-red-600 flex flex-col items-center justify-center text-white">
          <span className="text-xs font-medium">{dateMonth}月</span>
          <span className="text-lg font-bold leading-none">{dateDay}</span>
        </div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex-1 p-6 md:p-8">
            <div className="flex items-start gap-4">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-red-500 to-red-600 flex flex-col items-center justify-center text-white shadow-lg">
                <span className="text-sm">{dateMonth}月</span>
                <span className="text-2xl font-bold leading-none">{dateDay}</span>
              </div>
              <div className="flex-1">
...
const ServiceCard = ({ service, variant = 'default' }: { service: any; variant?: 'default' | 'grid' | 'list' }) => {
  if (variant === 'grid') {
    return (
      <motion.div
        whileHover={{ y: -4 }}
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <p className="text-amber-600 font-bold text-lg">
            ${service.price}
            <span className="text-stone-400 font-normal text-sm">/{service.unit}</span>
          </p>
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="pb-20 md:pb-0">
        ...
        <div className="md:hidden">
          {activeTab === "home" && (
            <MobileHome temple={templeData} events={events} services={services} />
          )}
          {activeTab === "services" && (
        ...
        <div className="hidden md:block">
          {activeSection === "home" && (
            <DesktopHome temple={templeData} events={events} services={services} gallery={gallery} />
          )}
...
const MobileHome = ({ temple, events, services }: any) => (
  <div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-4">
        <div className="grid grid-cols-4 gap-2">
          {QUICK_ACTIONS.map((action, i) => (
            ...
            >
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${action.color} flex items-center justify-center text-xl shadow-md`}>
                {action.icon}
              </div>
    ...
    <section className="mt-8 px-4 pb-8">
      ...
      <ContactInfo temple={temple} showCopy={true} variant="compact" />
    </section>
  </div>
);
      ...
      <section className="px-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {gallery.slice(0, 4).map((img: any, i: number) => (
            <div
              key={i}
              className={`rounded-xl overflow-hidden ${i === 0 ? "col-span-2 h-40" : "h-32"}`}
            >
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden">
              ...
              <div className="p-6">
                <ContactInfo temple={temple} showCopy={true} />
              </div>
            </div>
