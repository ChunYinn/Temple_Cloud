...
export function TemplePublicPage({ temple }: { temple: TempleData }) {
  ...
  const templeData = {
    ...
    stats: { followers: 1234, visitors: '10K+' },
    social: {
      facebook: '#',
      line: '#',
      instagram: '#',
  ...
  const services = [
    ...
    { id: 2, icon: '🐲', name: '太歲燈', price: '800', unit: '年', desc: '安太歲，消災解厄，保佑平安' },
    { id: 3, icon: '📿', name: '平安符', price: '100', unit: '個', desc: '隨身攜帶，趨吉避凶' },
    { id: 4, icon: '🙏', name: '祈福法會', price: '1,000', unit: '場', desc: '消災解厄，祈求平安' },
    { id: 5, icon: '🕯️', name: '藥師燈', price: '600', unit: '年', desc: '消災延壽，身體健康' },
    { id: 6, icon: '💫', name: '文昌燈', price: '500', unit: '年', desc: '金榜題名，學業進步' },
  ];
    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <header className="md:hidden sticky top-0 z-50 bg-gradient-to-r from-red-800 to-red-900 text-white shadow-lg">
        <div className="px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight">{templeData.name}</h1>
      ...
      <main className="pb-20 md:pb-0">
        <div className="md:hidden">
          {activeTab === 'home' && <MobileHome temple={templeData} events={events} services={services} />}
          {activeTab === 'services' && <MobileServices services={services} />}
          {activeTab === 'events' && <MobileEvents events={events} />}
          {activeTab === 'about' && <MobileAbout temple={templeData} gallery={gallery} />}
        </div>

        <div className="hidden md:block">
          {activeSection === 'home' && <DesktopHome temple={templeData} events={events} services={services} gallery={gallery} />}
          {activeSection === 'services' && <DesktopServices services={services} />}
          {activeSection === 'events' && <DesktopEvents events={events} />}
          {activeSection === 'about' && <DesktopAbout temple={templeData} gallery={gallery} />}
        </div>
      </main>
  ...
  <div>
    ...
    <div className="relative h-48">
      ...
      <div className="absolute bottom-4 left-4 right-4 flex gap-3">
        <div className="bg-white/20 backdrop-blur-md rounded-lg px-3 py-1.5 text-white text-sm">
          <span className="font-bold">{temple.stats.visitors}</span> 造訪
        </div>
      </div>
    </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-4">
        <div className="grid grid-cols-4 gap-2">
          {[
            ...
            >
              <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${action.color} flex items-center justify-center text-xl shadow-md transition-transform`}>
                {action.icon}
              </div>
              <span className="text-xs text-stone-600 font-medium">{action.label}</span>
            </motion.button>
          ))}
        </div>
      </div>
    ...
    <div className="px-4 mt-6">
      <p className="text-stone-600 leading-relaxed text-sm">
        {temple.intro || '歡迎來到' + temple.name + '，這裡是地方重要的信仰中心，提供各式祈福服務。'}
      </p>
    </div>
        ...
        {events.slice(0, 2).map((event: any) => (
          ...
          >
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-red-500 to-red-600 flex flex-col items-center justify-center text-white">
              <span className="text-xs font-medium">{event.date.split('/')[1]}月</span>
              <span className="text-lg font-bold leading-none">{event.date.split('/')[2]}</span>
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-stone-800 truncate">{event.title}</h3>
              <p className="text-stone-500 text-sm">{event.time} 開始</p>
            </div>
            <button className="px-3 py-1.5 bg-red-50 text-red-700 rounded-lg text-sm font-medium">報名</button>
          </motion.div>
        ))}
      </div>
    </section>
        ...
        {services.slice(0, 4).map((service: any) => (
          ...
          >
            ...
            <p className="text-amber-600 font-medium text-sm mt-1">
              ${service.price}<span className="text-stone-400">/{service.unit}</span>
            </p>
          </motion.div>
        ))}
      </div>
  ...
  <div className="px-4 py-6">
    ...
    <div className="bg-gradient-to-r from-amber-400 to-amber-500 rounded-2xl p-5 mb-6 shadow-lg">
      <div className="flex items-center gap-4">
        ...
        <button className="px-4 py-2 bg-stone-900 text-white rounded-xl font-medium">捐獻</button>
      </div>
    </div>
...
const MobileAbout = ({ temple, gallery }: any) => (
  <div className="pb-8">
    ...
    <section className="px-4 py-6">
      ...
      <p className="text-stone-600 leading-relaxed mt-4">{temple.fullDescription}</p>
    </section>
    ...
    <section className="px-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {gallery.slice(0, 4).map((img: any, i: number) => (
          <div key={i} className={`rounded-xl overflow-hidden ${i === 0 ? 'col-span-2 h-40' : 'h-32'}`}>
    ...
    <section className="px-4 mt-6">
      ...
      <div className="flex gap-3">
        {[
          { icon: '📘', label: 'Facebook', color: 'bg-blue-500' },
          { icon: '💬', label: 'LINE', color: 'bg-green-500' },
          { icon: '📷', label: 'Instagram', color: 'bg-pink-500' },
        ].map((social, i) => (
          <button key={i} className={`flex-1 ${social.color} text-white py-3 rounded-xl font-medium flex items-center justify-center gap-2`}>
          ...
          <div className="flex items-end justify-between">
            <div className="text-white">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h1 className="text-4xl lg:text-5xl font-bold">{temple.name}</h1>
                  <p className="text-amber-200 text-lg mt-1">{temple.subtitle}</p>
                </div>
              </div>
              <p className="text-white/80 max-w-2xl text-lg">
                {temple.intro || '歡迎來到' + temple.name + '，這裡是地方重要的信仰中心。'}
              </p>
            </div>
            ...
            <div className="hidden lg:flex gap-6">
              <div className="bg-white/10 backdrop-blur-md rounded-2xl px-6 py-4 text-white text-center">
                <div className="text-3xl font-bold">{temple.stats.visitors}</div>
                <div className="text-white/70 text-sm">造訪人次</div>
              </div>
              <div className="bg-white/10 backdrop-blur-md rounded-2xl px-6 py-4 text-white text-center">
                <div className="text-3xl font-bold">{temple.stats.followers}</div>
                <div className="text-white/70 text-sm">追蹤者</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="bg-white border-b border-stone-200 sticky top-16 z-40">
      <div className="max-w-6xl mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex gap-3">
            {[
              { icon: '💰', label: '線上香油錢', color: 'from-amber-500 to-amber-600', primary: true },
              { icon: '🪔', label: '點燈祈福', color: 'from-red-500 to-red-600' },
              { icon: '📅', label: '活動報名', color: 'from-blue-500 to-blue-600' },
            ].map((action, i) => (
              <button
                key={i}
          ...
          <div className="flex items-center gap-4 text-stone-500">
            {temple.phone && (
              <>
                <a href={`tel:${temple.phone}`} className="flex items-center gap-2 hover:text-stone-700">
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="text-red-700 font-medium hover:underline">查看全部活動 →</button>
            </div>
                  ...
                  <div className="p-5">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-stone-500 text-sm">{event.time} · {event.location}</p>
                      </div>
                    </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="text-red-700 font-medium hover:underline">查看全部服務 →</button>
            </div>
              ...
              {services.slice(0, 6).map((service: any) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {service.popular && (
                      <span className="px-2 py-0.5 bg-amber-100 text-amber-700 text-xs font-medium rounded-full">熱門</span>
                    )}
                  </div>
                  <h3 className="font-bold text-stone-800 text-lg">{service.name}</h3>
                  <p className="text-stone-500 text-sm mt-1 mb-3">{service.desc}</p>
                  <div className="flex items-center justify-between">
                    <p className="text-amber-600 font-bold text-lg">
                      ${service.price}<span className="text-stone-400 font-normal text-sm">/{service.unit}</span>
                    </p>
          ...
          <section>
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="text-red-700 font-medium hover:underline">查看更多 →</button>
            </div>
            <div className="grid grid-cols-4 gap-3">
              {gallery.slice(0, 4).map((img: any, i: number) => (
                <motion.div
                  ...
                  className={`rounded-2xl overflow-hidden cursor-pointer transition-opacity ${i === 0 ? 'col-span-2 row-span-2 h-64' : 'h-[120px]'}`}
                >
                  <img src={img.url} alt={img.caption} className="w-full h-full object-cover" />
                </motion.div>
              ))}
        ...
        <div className="space-y-6">
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-stone-900 font-bold text-xl mb-2">線上香油錢</h3>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-stone-100">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-lg bg-amber-100 flex items-center justify-center">⏰</div>
                <div>
              ...
              {temple.phone && (
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 rounded-lg bg-emerald-100 flex items-center justify-center">📞</div>
                  <div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-stone-100">
            ...
            <div className="space-y-2">
              {[
                { icon: '📘', label: 'Facebook 粉絲專頁', color: 'hover:bg-blue-50 hover:text-blue-600' },
                { icon: '💬', label: 'LINE 官方帳號', color: 'hover:bg-green-50 hover:text-green-600' },
                { icon: '📷', label: 'Instagram', color: 'hover:bg-pink-50 hover:text-pink-600' },
              ].map((social, i) => (
                <button key={i} className={`w-full py-3 px-4 rounded-xl font-medium flex items-center gap-3 border border-stone-200 text-stone-700 transition-colors ${social.color}`}>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="text-stone-400 text-sm text-center">
            <p>由 🏮 廟務雲 Temple Cloud 技術支援</p>
            <p className="mt-1">© 2024 {temple.name}. All rights reserved.</p>
          </div>
        </div>
      </div>
    </footer>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-stone-800/70 mt-1">隨喜捐獻，累積功德，護持道場</p>
          </div>
        </div>
      ...
      {services.map((service: any) => (
        ...
        >
          <div className="flex items-start justify-between mb-4">
            ...
            {service.popular && (
              <span className="px-3 py-1 bg-amber-100 text-amber-700 text-sm font-medium rounded-full">熱門</span>
            )}
          </div>
          <h3 className="font-bold text-stone-800 text-xl mb-2">{service.name}</h3>
          <p className="text-stone-500 mb-4">{service.desc}</p>
          <div className="flex items-center justify-between pt-4 border-t border-stone-100">
            <p className="text-amber-600 font-bold text-2xl">
              ${service.price}<span className="text-stone-400 font-normal text-base">/{service.unit}</span>
            </p>
          ...
          <div className="md:flex">
            ...
            <div className="flex-1 p-6 md:p-8">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-stone-800 text-xl mb-2">{event.title}</h3>
      ...
      <div className="grid lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2">
          <section className="mb-12">
            ...
            <div className="prose prose-stone max-w-none">
              ...
              <p className="text-stone-600 leading-relaxed text-lg mt-4">{temple.fullDescription}</p>
            </div>
          </section>
            ...
            <div className="grid grid-cols-3 gap-4">
              {gallery.map((img: any, i: number) => (
                <motion.div
                  key={i}
                  whileHover={{ scale: 1.05 }}
                  className={`rounded-2xl overflow-hidden cursor-pointer transition-opacity ${i === 0 ? 'col-span-2 row-span-2 h-80' : 'h-40'}`}
                >
                  <img src={img.url} alt={img.caption} className="w-full h-full object-cover" />
                </motion.div>
              ))}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-stone-200 rounded-2xl h-64 flex items-center justify-center">
            <span className="text-stone-400">🗺️ Google Maps</span>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-stone-100">
            ...
            <div className="flex gap-3">
              {['📘', '💬', '📷'].map((icon, i) => (
                <button key={i} className="flex-1 py-3 border border-stone-200 rounded-xl text-xl hover:bg-stone-50 transition-colors">
                  {icon}
                </button>
              ))}
            </div>
    ...
    <div className="bg-gradient-to-r from-red-600 to-red-700 p-4 text-white">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-white/80 text-sm mt-0.5">{event.date} {event.time}</p>
        </div>
      </div>
    </div>
    <div className="p-4">
